...
import com.rfid.reader.databinding.ActivityInventoryScanNormalBinding
...
/**
 * Activity per scansione RFID - Modalità NORMAL
 * Mostra solo counter Principale e "Validated"
 * Rispetta setting "Tutti" o "Solo Censiti"
 */
class InventoryScanNormalActivity : AppCompatActivity() {
    private lateinit var binding: ActivityInventoryScanNormalBinding
    ...
    companion object {
        private const val TAG = "InventoryScanNormal"
        private const val PERMISSION_REQUEST_CODE = 1001
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityInventoryScanNormalBinding.inflate(layoutInflater)
        ...
        android.util.Log.d(TAG, "Opening NORMAL inventory scan: $inventoryId - $inventoryName")
    ...
    private fun setupObservers() {
        ...
        // Contatore Validated (solo per Normal mode)
        viewModel.expectedCount.observe(this) { count ->
            binding.tvValidatedCount.text = count.toString()
    ...
    private fun setupListeners() {
        ...
        binding.btnInfo.setOnClickListener {
            android.util.Log.d(TAG, "Opening details for inventory $inventoryId (Normal mode)")
            ...
            intent.putExtra("INVENTORY_MODE", "normal")
    ...
    private fun showClearConfirmDialog() {
        AlertDialog.Builder(this)
            ...
            .setMessage("Svuotare l'inventario? Tutti i tag scannerizzati verranno rimossi.")
            .setPositiveButton("Conferma") { _, _ ->
                android.util.Log.d(TAG, "Clearing inventory $inventoryId")
                lifecycleScope.launch {
                    viewModel.clearInventory()
                    Toast.makeText(this@InventoryScanNormalActivity, "Inventario svuotato", Toast.LENGTH_SHORT).show()
                }
            }
    ...
    private fun showCloseConfirmDialog() {
        AlertDialog.Builder(this)
            ...
            .setMessage("Chiudere l'inventario? Non sarà più possibile aggiungere tag.")
            .setPositiveButton("Conferma") { _, _ ->
                android.util.Log.d(TAG, "Closing inventory $inventoryId")
                lifecycleScope.launch {
                    viewModel.closeInventory()
                    Toast.makeText(this@InventoryScanNormalActivity, "Inventario chiuso", Toast.LENGTH_SHORT).show()
                    finish()
                }
            }
            .setNegativeButton("Annulla", null)
    ...
    private fun showDeleteConfirmDialog() {
        AlertDialog.Builder(this)
            ...
            .setMessage("Eliminare l'inventario? Questa azione è irreversibile.")
            .setPositiveButton("Conferma") { _, _ ->
                android.util.Log.d(TAG, "Deleting inventory $inventoryId")
                lifecycleScope.launch {
                    viewModel.deleteInventory()
                    Toast.makeText(this@InventoryScanNormalActivity, "Inventario eliminato", Toast.LENGTH_SHORT).show()
                    finish()
                }
            }
            .setNegativeButton("Annulla", null)
